...
def snapkv_compress_gqa(query_states, key_states, value_states,
    ...
    # Q: [bsz, kv_heads, groups, seq, head_dim] → 평균 → [bsz, kv_heads, seq, head_dim]
    q_grouped = query_states.view(bsz, num_kv_heads, num_groups, q_len, head_dim)
    q_mean = q_grouped.mean(dim=2)

    # 관측 윈도우의 Q로 전체 K에 대해 attention score 계산
...
def patch_llama_attention(window_size=64, max_capacity_prompt=512, kernel_size=5,
    ...
    def new_forward(self, hidden_states, position_embeddings=None,
        ...
        if is_prefill and seq_len > max_capacity_prompt:
            # ✅ 핵심 수정:
            # 1. Full KV로 attention 계산 (causal mask와 shape 일치)
            attn_output, attn_weights = eager_attention_forward(
                self, query_states, key_states, value_states,
                attention_mask, dropout=0.0, scaling=self.scaling, **kwargs,
            )
            # 2. 압축 후 cache에 저장
            key_compressed, value_compressed = snapkv_compress_gqa(
                query_states, key_states, value_states,
                window_size, max_capacity_prompt, kernel_size,
                num_q_heads, num_kv_heads
            )
            if past_key_values is not None:
                past_key_values.update(key_compressed, value_compressed, self.layer_idx)
        else:
            # decode 단계: 압축된 cache 사용
            if past_key_values is not None:
...
def main():
    ...
    with open("results_snapkv_v4.json", "w") as f:
