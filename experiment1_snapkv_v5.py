...
TEST_CASES = [
    ...
    },
    {
        ...
        request can be stored in non-contiguous memory spaces. The PagedAttention algorithm divides 
        the KV cache into fixed-size blocks, each containing the key-value pairs for a fixed number 
        of tokens. The implementation uses a block size of 16 tokens per block by default.
        """ * 20,
...
def snapkv_compress_gqa(query_states, key_states, value_states,
    ...
    # Q 按 kv_head 分组取平均: [bsz, kv_heads, seq, head_dim]
...
def compress_cache_after_prefill(past_key_values, query_states_per_layer,
                                  window_size, max_capacity_prompt,
                                  num_q_heads=32, num_kv_heads=8):
    """
    Prefill 结束后，直接修改 cache.layers[i].keys/values
    这样 get_seq_length() 也会反映压缩后的长度
    """
    compressed_count = 0
    for i, layer in enumerate(past_key_values.layers):
        k = layer.keys
        v = layer.values
        q = query_states_per_layer[i]

        if k.shape[2] > max_capacity_prompt:
            k_new, v_new = snapkv_compress_gqa(
                q, k, v, window_size, max_capacity_prompt,
...
def patch_llama_attention_capture(window_size, max_capacity_prompt,
                                   num_q_heads=32, num_kv_heads=8):
    """
    在 forward 里捕获每层的 query_states，存到 module 上
    压缩在 prefill 后统一做
    """
...
def run_inference(model, tokenizer, context, question,
    ...
    with torch.no_grad():
        ...
        if use_snapkv and input_len > max_capacity_prompt:
            # 收集每层的 query
            queries = [layer.self_attn._last_query for layer in model.model.layers]
            n = compress_cache_after_prefill(
                past_kv, queries, window_size, max_capacity_prompt
            )
            new_seq_len = past_kv.layers[0].keys.shape[2]
            print(f"  Compressed: {input_len} → {new_seq_len} tokens ({n} layers)")
...
def main():
    ...
    all_results = []
    orig = patch_llama_attention_capture(64, 512)
    ...
    for cfg in configs:
        ...
        for i, test in enumerate(TEST_CASES):
            ...
            result = run_inference(
                model, tokenizer, test["context"], test["question"],
                use_snapkv=True,
    ...
    with open("results_snapkv_v5.json", "w") as f:
    ...
    for r in all_results:
        print(f"\n{r['config']}:")
        for c in r["cases"]:
            print(f"  {'✅' if c['correct'] else '❌'}  {c['question'][:50]}")
            print(f"     time={c['time']:.2f}s (base={c['baseline_time']:.2f}s)  mem={c['peak_mem_gb']:.2f}GB")
